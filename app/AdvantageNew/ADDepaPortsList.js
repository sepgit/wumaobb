...
class ADDepaPortsList extends Component {
  constructor(props) {
    super(props);
  ...
  getDepaPortList(){
    ...
    let url = 'api/ports/?userName='+userName+'&wxtoken='+wxtoken+'&rowCount=0&pageIndex=1&type=1&serv='+serv;
    let params = [];
  ...
  getRecentPortList(){
    ...
    let wxtoken = this.props.wxtoken;
    let serv = this.props.serv;
    let url = 'api/ports/?userName='+userName+'&wxtoken='+wxtoken+'&rowCount=0&pageIndex=1&type=1&recent=true&serv='+serv;
    let params = [];
  ...
  SearchonChange(event){
    ...
    if (v!=''){
      for (let i=0;i<this.state.DepaPortList.length;i++)
      {
        chv=this.state.DepaPortList[i].portName + this.state.DepaPortList[i].chsName;
        chv=chv.toUpperCase();
        if (chv.indexOf(v)!=-1){
          Ob={port:this.state.DepaPortList[i].port, portName:this.state.DepaPortList[i].portName, chsName:this.state.DepaPortList[i].chsName};
...
export default ADDepaPortsList;